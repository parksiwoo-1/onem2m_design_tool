      ...
      <li :class="{ resourceBox: true, nestTree: this.nestTree }">
        <div class="nestedBox">
          ...
          >
            ...
            <!-- CIN count warning -->
            <span v-if="element.childCount > 100" class="warningBadge" :title="`${element.childCount} CIN resources`">
              ⚠️ {{ element.childCount }}
            </span>
          </p>

          <!-- CSE 오른쪽 토글: 서버에서 로드한 경우에만 표시 -->
          <button
            v-if="element.ty === 5 && element.allResourcesLoaded && element.tasks && element.tasks.length > 0"
            class="toggleBtn toggleBtnRight"
            :style="getToggleStyle(element.ty)"
            @click.stop="$emit('toggle-expand', element, 'right')"
            :title="element.allCollapsed ? 'Expand all' : 'Collapse all'"
          >
            {{ element.allCollapsed ? '+' : '−' }}
          </button>
          <!-- 일반 리소스의 오른쪽 토글: 서버에서 로드했고 hasChildren이 true인 경우만 표시 -->
          <button
            v-else-if="element.createdOnServer && element.hasChildren"
            class="toggleBtn"
            :style="getToggleStyle(element.ty)"
        ...
        <!-- Show all resources -->
        <nested-draggable
          v-if="(element.ty === 5 ? !element.allCollapsed : element.expanded) && element.tasks && getChildRT(element.ty).length > 0"
          :tasks="element.tasks"
          :group="this.group"
